...
pub struct MarginStatus {
    /// Collateral reserved before the current event's refresh replacement.
    pub current_margin: Money,
    /// Total collateral required by the current event's held-position facts.
    pub required_margin: Money,
    /// Non-negative cash shortfall, `max(required_margin - cash, 0)`.
    pub deficit: Money,
}
